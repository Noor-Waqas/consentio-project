...
<?php 
	if (!empty($questions) && isset($questions[0])){		
		$showcomment = $questions[0]->is_locked  == 1 ? true : false ;
		$asset_detail = DB::table('assets')->find($questions[0]->asset_id);
	...
	// $all_file = ['Images', '.docs', '.pdf', '.xlxs , .csv', '.zip'];
		...
		<div class="container-fluid">
			...
			<div class="main-panel">
				...
				<div class="collapseZero" id="form-body">
					...
					@if($showcomment)
						<button type="button" class="open_add_remidation_model_btn btn btn-primary" q-id=""> Add Remidation Plan </button>
					@endif
					</div>
										...
										switch ($type):
													...
													@if($showcomment)
														<div class="col-md-12 py-3">
															<div class="w-100 mr-3">
																...
																<select class="form-control" class="add_rating_in_db" onchange="add_question_rating_in_db(event)" q-id="{{ $question->q_id }}" form_id="{{ $question->uf_id }}">
																	@if(isset($filled[$question->form_key]) && $filled[$question->form_key]['rating'] == 0)
																		<option value="">-- SELECT Assessment --</option>
																	@endif
																	@foreach($eval_ratings as $rate)
																	<option value="{{ $rate->id }}" style=" background-color: {{$rate->color}}"  @if($filled[$question->form_key]['rating'] ==  $rate->id) selected @endif>{{ $rate->assessment }}</option>
																	@endforeach
																</select>
															</div>
														</div>
														<div class="col-md-12 py-3">
															...
															<textarea rows="4"   class="form-control comment_for_question" placeholder="Comment ..."  q-id="{{ $question->q_id }}" form_id="{{ $question->uf_id }}"><?php echo (isset($filled[$question->form_key]))?($filled[$question->form_key]['admin_comment']):('') ?></textarea>
															<!-- <button type="button" class="open_add_comment_model_btn btn btn-primary mr-2"  > Add comment</button> -->
														</div>
													@endif
												<?php
												$total_questions++;														
													...
													<section class="options"> 
														...
														@if($question->attachment_allow)
															<div class="pt-2">
																...
																<form id="question_with_attachment_{{ $question->q_id }}" enctype="multipart/form-data" method="POST">
																	...
																	<input type="hidden" name="accepted_types" id="file_accepted_types_{{$question->q_id}}" value="{{$question->attachments}}">
																	<input type="file" name="img-{{ $question->q_id }}" q-id="{{ $question->q_id }}" id="file-upload-{{$question->q_id}}" class="attachment_file" {{(isset($filled[$question->form_key]['attachment']) && !empty($filled[$question->form_key]['attachment']))?("data-default-file=".URL::to('public/'.$filled[$question->form_key]['attachment'])):('') }}>
											...
											case('im'): ?>
												<form id="upload_form-{{ $question->q_id }}" enctype="multipart/form-data" method="POST">
													...
													<span id="image_error{{$question->q_id}}"></span>
												</form> 
												@if($showcomment)
													<div class="col-md-12 py-3">
														<div class="w-100 mr-3">
			...
			<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
				...
				<form class="w-100">
					<div class="modal-body">
						...
						<div class="form-group py-2">
							...
							<select class="form-control" id="asset_id">
								<option value="{{ $asset_detail->id }}">{{ $asset_detail->name }}</option>
							</select>
						</div>
						...
						<div class="form-group py-2">
							...
							<option value="{{ $asset_detail->id }}">{{ $asset_detail->business_unit }}</option>
							</select>
						</div>

						<div class=" form-group py-2">
							<label for="comment_for_question">Control name</label>
							<select class="form-control" id="control_name">
								<option value="">-- SELECT --</option>
								<option value="123"> Testing Value </option>
						...
						<div class=" form-group py-2">
							...
							<select class="form-control" id="initial_review">
								<option value="">-- SELECT --</option>
						...
						<div class=" form-group py-2">
							...
							<input type="" id="proposed_remediation" value="" class="form-control" required>
						</div>

						<div class=" form-group py-2">
							<label for="comment_for_question">Completed Actions</label>
							<input type="" id="completed_actions" value="" class="form-control" required>
						</div>

						<div class=" form-group py-2">
							<label for="comment_for_question">Initial Rating (Automatic)</label>
							<select class="form-control" id="initial_rating">
								<option value="">-- SELECT --</option>
							</select>
						</div>
						...
						<div class=" form-group py-2">
							...
							<input type="date" id="eta" value="" class="form-control" required>
						</div>

						<div class=" form-group py-2">
							<label for="comment_for_question">Person in Charge</label>
							<input type="" id="person_in_charge" value="" class="form-control" required>
						</div>
						...
						<div class="form-group py-2">
							...
							<select class="form-control" id="post_remediation_rating">
								<option value="">-- SELECT --</option>
							</select>
						</div>
					...
					<div class="modal-footer">
						<b id="error_for_remidation" class="text-danger"></b>
						<button type="button" class="btn btn-outline-danger" data-dismiss="modal">Cancel</button>
						<button type="submit" class="btn btn-primary" id="add_remidation_to_db"> Add Reidation</button>
					</div>
					</div>
				</form>
			</div>
		...
		<script>
			function add_question_rating_in_db(event) {
				const data = {
					...
					f_id: $(event.target).attr("form_id") ,
				}
				$.ajaxSetup({
					headers: {
			...
			$("#add_remidation_to_db").on('click', function (event) {
				event.preventDefault();
				// let asset_id 				=  $("#asset_id").val();
					// let business_unit   		=  $("#business_unit").val();
					// let control_name    		=  $("#control_name").val();
					// let initial_review  		=  $("#initial_review").val();
					// let proposed_remediation  	=  $("#proposed_remediation").val();
					// let completed_actions       =  $("#completed_actions").val();
					// let initial_rating 			=  $("#initial_rating").val();
					// let eta  					=  $("#eta").val();
					// let person_in_charge        =  $("#person_in_charge").val();
					// let status  				=  $("#status").val();
					// let post_remediation_rating =  $("#post_remediation_rating").val();

				let data = {
					asset_id 				: $("#asset_id").val(),
					business_unit   		: $("#business_unit").val(),
					control_name    		: $("#control_name").val(),
					initial_review  		: $("#initial_review").val(),
					proposed_remediation  	: $("#proposed_remediation").val(),
					completed_actions       : $("#completed_actions").val(),
					initial_rating 			: $("#initial_rating").val(),
					eta  					: $("#eta").val(),
					person_in_charge        : $("#person_in_charge").val(),
					status  				: $("#status").val(),
					post_remediation_rating : $("#post_remediation_rating").val(),
				}
				...
				$.ajax({
					url:'{{route('add_remediation')}}',
					method:'POST',
					data: data,
					success: function(response) {
						if (response.status == 400) {
							$("#error_for_remidation").html(response.error)
						}
