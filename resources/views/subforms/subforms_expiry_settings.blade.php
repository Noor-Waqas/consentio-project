...
<div class="card custom_width_sub_card">
    ...
    <div id="sar-settings " class="card-body text">
        <div class="col col-md-6">
            ...
            <div class="form-group">
                ...
                <select name="period"  class="form-control" id="period">
                    <option value="days"  <?php echo ($subform_settings->period == 'days')?('selected'):(''); ?>>{{ __('Days') }}</option>
...
$(document).ready(function() {
    ...
    $('#update-settings').click(function (){
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.status == 'success') {
              swal( "{!! __('Sub Form Settings Updated') !!}", "{!! __('These settings will be applied from the next form assignment to users') !!}" , 'success');
            }
            else {
        ...
        // alert('asdasdasd');
              ...
              // swal('Invalid Duration Value','Please enter valid duration', 'failure');                
