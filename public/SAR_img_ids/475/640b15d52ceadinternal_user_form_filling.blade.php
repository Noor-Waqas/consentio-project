...
@endsection
	...
	<body>
                                    ...
                                    <div class="content p-3">
                                        @if(session('locale')=='fr')
                                            ...
                                            @if($question->question_comment_fr)
                                                <h6 class="question-comment"><small class="mr-3">En Comment:<span class="edit_comment">{{ $question->question_comment_fr }}</span></small></h6>
                                            @endif   
                                        @else
                                            <h6> {{$question->question_num}}. {{ $question->question }} </h6>    
                                            @if($question->question_comment)
                                                <h6 class="question-comment mt-0"><small class="mr-3">En Comment:<span class="edit_comment">{{ $question->question_comment }}</span></small></h6>
                                            @endif 
                                        @endif
													...
													<div class="col-md-12 py-3">
														<div class="w-100 mr-3">
															...
															<select class="form-control" class="add_rating_in_db" onchange="add_question_rating_in_db(event)" q-id="{{ $question->id }}">
																...
																<option value="{{ $rate->id }}" style=" background-color: {{$rate->color}}"  @if($filled_info[$question->id]['rating'] ==  $rate->id) selected @endif>{{ $rate->assessment }}</option>
																@endforeach
															</select>
														</div>
													</div>
		...
		{{-- Remediation Plan Modal --}}
			<div class="modal fade" id="add_remediation_model" tabindex="-1" role="dialog" aria-labelledby="add_remediation_model_title" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
					<div class="modal-content">
					<div class="modal-header">
					<h4 class="modal-title" id="add_remediation_model_title">Remediation Plan</h4>
					...
					<div class="modal-body">
						<div class="row">
							<div class="col-md-6"> 
								<h5>{{__("remediation_asset_name")}}: @if(session('locale') == 'fr') {{ $asset->name }} @else {{ $asset->name }} @endif </h5>
							</div>
							<div class="col-md-6"> 
								<h5>{{__("remediation_asset_business_unit")}}: @if(session('locale') == 'fr') {{ $asset->business_unit }} @else {{ $asset->business_unit }} @endif </h5>
							</div>
						</div>

						<div class="row">
							<div class="col-12">
								<div class="w-100 p-4 mt-3">
									<h6>Controls with marginal and Weak Ratings</h6>
									<div class="px-2" id="append_questions"></div>
								</div>
							</div>
						</div>
					...
					<button type="button" class="btn btn-primary">Add Remediation</button>
					</div>
					</div>
				</div>
			</div>
			...
			function counts(id = 0){
				...
				$.ajax({
					...
					success: function(response) {
						...
						if (response.total_questions == response.added_ratting) {
							$('#show_remediation_plan').html(`<button class="btn btn-primary" onclick=show_add_remediation_model()>{{__('Add Remediation plan')}}</button>`)
						}
						if (locked != 1) {
